...
- [~] **SW-01**: A `switch` nginx service is the client's only endpoint, reachable at `app.demo.test` on HTTP 9092 and SSH 22 — unchanged from the client's point of view versus v1 *(HTTP 9092 delivered in Phase 5; the switch's SSH:22 listener lands in Phase 6 with SW-03, at which point this is fully true)*
...
| SW-01 | Phase 5 (HTTP) / Phase 6 (SSH) | Partial — HTTP re-homed; SSH:22 at the switch completes in Phase 6 (SW-03) |
